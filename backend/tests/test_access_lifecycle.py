...
def test_full_access_lifecycle(db_session, client):
    citizen_headers = {"X-Dev-User-Email": "raghavendra@civicone.gov.in"}
    bank_headers = {"X-Dev-User-Email": "loan_officer@abcbank.com"}

    # 1. Citizen lists access requests to get the pending request ID
    res_list = client.get("/api/v1/access-requests", headers=citizen_headers)
    assert res_list.status_code == 200
    requests = res_list.json()
    pending_req = [r for r in requests if r["status"] == "PENDING"][0]
    req_id = pending_req["id"]

    # 2. Citizen approves the pending request
    ...
    # 3. Bank queries authorized data for citizen Raghavendra
    ...
    grant_to_revoke = [g for g in active_grants if g["access_request_id"] == req_id and g["status"] == "ACTIVE"][0]
