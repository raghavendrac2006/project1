...
function buildTimeline(): CivicMilestone[] {
  ...
  if (identity) {
    milestones.push({
      ...
      description: `Your SAMAGRA Sovereign Digital ID was issued and biometric verification completed. Verification level: ${identity.status}.`,
      type: 'identity',
      icon: '🪪',
      isHighlight: true,
    })
  ...
  if (family.length > 0) {
    milestones.push({
      ...
      description: `${family.length} family member${family.length > 1 ? 's' : ''} added to your SAMAGRA Family Vault.`,
      type: 'family',
      icon: '👨‍👩‍👧',
      isHighlight: false,
    })
